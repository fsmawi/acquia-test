...
module.exports = function () {
  ...
  this.Then(/^I should be shown appropriate \|(.*?)\| in summary table$/, function (countUpTimeIdentifier) {
    boostrap(this.browser);
    return jobDetailPage.getCountUpTime(page.getDynamicValue(countUpTimeIdentifier))
      .then((countUpTime) => {
        return expect(countUpTime.match('[0-9]?[0-9]:[0-9][0-9]')).to.not.be.null;
      });
  });
};
